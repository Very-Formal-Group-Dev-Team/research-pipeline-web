...
import type { DefenseType } from '@/lib/api/coordinator';
...
export default function AdviserRubricPage() {
  const { user, handleLogout } = useDashboardUser('Adviser');
  const [rubrics, setRubrics] = useState<AdviserRubric[]>([]);
  const [loading, setLoading] = useState(true);

  const [editorOpen, setEditorOpen] = useState(false);
  const [editingId, setEditingId] = useState<string | null>(null);

  const [deleteTarget, setDeleteTarget] = useState<AdviserRubric | null>(null);
  const [deleting, setDeleting] = useState(false);
  const [deleteError, setDeleteError] = useState('');

  async function loadRubrics() {
    setLoading(true);
    const res = await getAdviserRubrics();
  ...
  function openEdit(rubric: AdviserRubric) {
  ...
  async function handleDelete() {
    ...
    setDeleting(true);
    setDeleteError('');
    const res = await deleteAdviserRubric(deleteTarget.id);
    setDeleting(false);
    if (res.error) {
