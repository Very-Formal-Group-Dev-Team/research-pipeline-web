...
export const dayCellVariants = cva("text-white", {
  variants: {
    color: {
      ...
      red: "bg-error-500 text-white hover:bg-error-600",
      yellow: "bg-warning-500 text-white hover:bg-warning-600",
      purple:
...
export function DayCell({ cell, events, eventPositions }: IProps) {
  ...
  const cellContent = useMemo(
    () => (
      <motion.div
        className={cn(
          "flex h-full min-h-20 sm:min-h-28 lg:min-h-40 flex-col gap-1 border-l border-t",
      ...
      >
        <DroppableArea date={date} className="w-full h-full py-2">
          ...
          >
            {cellEvents.length === 0 && !isMobile && !readOnly ? (
